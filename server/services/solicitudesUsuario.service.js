...
async function getDetalleSolicitud(idSolicitud, login) {
  ...
  // Verificar ownership — excluye desarrollos
  const check = await pool
    .request()
    .input("id", sql.Int, idSolicitud)
    .input("login", sql.VarChar, login)
    .query(
      `SELECT idSolicitud FROM solicitudTI
       WHERE idSolicitud = @id AND idUsuario = @login AND idServicio != 2`,
    );
...
async function postComentario(idSolicitud, login, nombreUsuario, comentario) {
  ...
  const check = await pool
    ...
    .input("login", sql.VarChar, login)
    .query(
      `SELECT idSolicitud, tecnicoAsignado, folio
       FROM solicitudTI
       WHERE idSolicitud = @id AND idUsuario = @login AND idServicio != 2`,
    );
...
async function cancelarSolicitud(idSolicitud, login) {
  ...
  const check = await pool
    ...
    .input("login", sql.VarChar, login)
    .query(
      `SELECT idEstatus FROM solicitudTI
       WHERE idSolicitud = @id AND idUsuario = @login AND idServicio != 2`,
    );
...
async function postEvaluacion(
  ...
  const check = await pool
    ...
    .input("login", sql.VarChar, login)
    .query(
      `SELECT idEstatus, tecnicoAsignado, folio
       FROM solicitudTI
       WHERE idSolicitud = @id AND idUsuario = @login AND idServicio != 2`,
    );
